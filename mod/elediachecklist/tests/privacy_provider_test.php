...
use mod_elediachecklist\privacy\provider;

defined('MOODLE_INTERNAL') || die();

/**
 * Class mod_elediachecklist_privacy_provider_testcase
 */
class mod_elediachecklist_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    protected function setUp(): void {
        ...
        // Create 4 checklists.
        /** @var mod_elediachecklist_generator $plugingen */
        $plugingen = $gen->get_plugin_generator('mod_elediachecklist');
        $params = [
        ...
        foreach ($this->checklists as $checklist) {
            ...
            foreach ($itemtexts as $text) {
                $item = new \mod_elediachecklist\local\checklist_item([], false);
        ...
        /** @var \mod_elediachecklist\local\checklist_item[] $items */
        $items = \mod_elediachecklist\local\checklist_item::fetch_all(['checklist' => $this->checklists[0]->id], true);
        ...
        $items = \mod_elediachecklist\local\checklist_item::fetch_all(['checklist' => $this->checklists[2]->id], true);
        $items = array_values($items);
        $comment = new \mod_elediachecklist\local\checklist_comment([], false);
    ...
    public function test_get_metadata(): void {
        $collection = new collection('mod_elediachecklist');
        ...
        $this->assertArrayHasKey('eledia', $privacyfields);
        ...
        foreach ($privacyfields as $field) {
            get_string($field, 'mod_elediachecklist');
        }
        get_string($table->get_summary(), 'mod_elediachecklist');
    ...
    public function test_get_contexts_for_userid(): void {
        $cms = [
            get_coursemodule_from_instance('elediachecklist', $this->checklists[0]->id),
            get_coursemodule_from_instance('elediachecklist', $this->checklists[1]->id),
            get_coursemodule_from_instance('elediachecklist', $this->checklists[2]->id),
            get_coursemodule_from_instance('elediachecklist', $this->checklists[3]->id),
        ];
        $expectedctxs = [
    ...
    public function test_export_for_context(): void {
        ...
        $this->export_context_data_for_user($this->student->id, $ctxs[0], 'mod_elediachecklist');
        $writer = \core_privacy\local\request\writer::with_context($ctxs[0]);
        $this->assertTrue($writer->has_any_data());

        $this->export_context_data_for_user($this->student->id, $ctxs[1], 'mod_elediachecklist');
        $writer = \core_privacy\local\request\writer::with_context($ctxs[1]);
        $this->assertTrue($writer->has_any_data());

        $this->export_context_data_for_user($this->student->id, $ctxs[2], 'mod_elediachecklist');
        $writer = \core_privacy\local\request\writer::with_context($ctxs[2]);
        $this->assertTrue($writer->has_any_data());

        $this->export_context_data_for_user($this->student->id, $ctxs[3], 'mod_elediachecklist');
    ...
    public function test_delete_data_for_all_users_in_context(): void {
        ...
        /** @var \mod_elediachecklist\local\checklist_item[] $items */
        $items = \mod_elediachecklist\local\checklist_item::fetch_all(['checklist' => $this->checklists[1]->id]);
        $items = array_values($items);
        ...
        $cm = get_coursemodule_from_instance('elediachecklist', $this->checklists[0]->id);
        ...
        $cm = get_coursemodule_from_instance('elediachecklist', $this->checklists[1]->id);
        $cmcontext = context_module::instance($cm->id);
        provider::delete_data_for_all_users_in_context($cmcontext);
        ...
        $cm = get_coursemodule_from_instance('elediachecklist', $this->checklists[2]->id);
        $cmcontext = context_module::instance($cm->id);
        provider::delete_data_for_all_users_in_context($cmcontext);
    ...
    public function test_delete_data_for_user(): void {
        ...
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->student, 'eledia',
                                                                            [$ctxs[0]->id]);
        ...
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->student, 'eledia', $contextids);
        provider::delete_data_for_user($contextlist);
    ...
    public function test_get_users_in_context(): void {
        ...
        $userlist = new \core_privacy\local\request\userlist($ctxs[0], 'mod_elediachecklist');
        provider::get_users_in_context($userlist);
        $this->assertCount(1, $userlist);

        $userlist = new \core_privacy\local\request\userlist($ctxs[1], 'mod_elediachecklist');
        provider::get_users_in_context($userlist);
        $this->assertCount(2, $userlist);

        $userlist = new \core_privacy\local\request\userlist($ctxs[2], 'mod_elediachecklist');
        provider::get_users_in_context($userlist);
        $this->assertCount(1, $userlist);

        $userlist = new \core_privacy\local\request\userlist($ctxs[3], 'mod_elediachecklist');
        provider::get_users_in_context($userlist);
    ...
    public function test_delete_data_for_users(): void {
        ...
        $approvedlist = new \core_privacy\local\request\approved_userlist($ctxs[0], 'mod_elediachecklist', [$this->student->id]);
        ...
        $approvedlist = new \core_privacy\local\request\approved_userlist($ctxs[1], 'mod_elediachecklist', [$this->student->id]);
        provider::delete_data_for_users($approvedlist);
